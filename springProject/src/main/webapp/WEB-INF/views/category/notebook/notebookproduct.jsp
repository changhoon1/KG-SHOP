        ...
        <!-- /.row -->
         <ul class="pagination">
		    <c:if test="${pageMaker.prev}">
		    	<li class="page-item"><a class="page-link" href="notebookproduct${pageMaker.makeSearch(pageMaker.startPage - 1)}&product_category_no=${product_category_no}">이전</a></li>
		    </c:if> 
			
		    <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
		    	<li class="page-item" <c:out value="${pageMaker.cri.page == idx ? 'class=info' : ''}"/>>
		    	<a class="page-link" href="notebookproduct${pageMaker.makeSearch(idx)}&product_category_no=${product_category_no}">${idx}</a></li>
		    </c:forEach>
		
		    <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
		    	<li class="page-item"><a class="page-link" href="notebookproduct${pageMaker.makeSearch(pageMaker.endPage + 1)}&product_category_no=${product_category_no}">다음</a></li>
		    </c:if> 
		  </ul>
